...
<div class="pd-20">
  <form action="" method="post">
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="/pages/corporation-add.jsp" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加</a></span>  <span class="r">共有数据：<strong>${list.size()}</strong> 条</span> </div>
    <div class="mt-20">
      <table class="table table-border table-bordered table-bg">
        ...
        <tbody>
        <c:forEach items="${list}" var="copr" varStatus="status">
          <tr class="text-c">
            ...
            <td>${copr.address}</td>
            <td>${copr.telNum}</td>
            <td><c:if test="${copr.authentic==1}"><a style="color: #008200">已审核</a></c:if>
              <c:if test="${copr.authentic==0}"><a style="color: #cc0000">未审核</a></c:if></td>
            <td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5" href="/getCorporationInfoById.do?id=${copr.corpId}"  title="查看"><i class="Hui-iconfont">&#xe665;</i></a> <a style="text-decoration:none" onclick="delconfirm(${copr.corpId},${copr.role})" class="ml-5" href="javascript:void(0)" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
          </tr>
        </c:forEach>
        </tbody>
      </table>
...
<script type="text/javascript" src="../resources/js/corporation.js"></script>
