...
<div class="pd-20">
	<form action="/getAllAdmin.do" method="post">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="/toaddAdmin.do" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a></span> <span class="r">共有数据：<strong>${list.size()}</strong> 条</span> </div>
		<div class="mt-20">
		...
		<c:forEach items="${list}" var="admin" varStatus="status">
			<tr class="text-c">
				...
				<td>${map.get(admin.corpId)}</td>
				<td>${admin.loginTime}</td>
				<td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5" href="/toupdateAdmin.do?id=${admin.userId}"  title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" onclick="delconfirm(${admin.userId})" class="ml-5" href="javascript:void(0)" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
